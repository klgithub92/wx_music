...
Page({
  ...
  async getVideoGroupListData() {
    ...
    this.setData({
      ...
      navId: videoGroupListData.data[0].id // 默认第一个被选中
    })

    // 获取视频列表数据 必须在这里因为是异步函数
    this.getVideoList(this.data.navId)
  },

  // 获取视频列表数据
  async getVideoList(navId) {
    if (!navId) return // 判断navId为空的情况
    let videoListData = await request('/video/group', { id: navId })
    // console.log(videoList) // 要得到cook值 确保登录了
    // 手动关闭视频加载提示框
    wx.hideLoading()
  ...
  // 切换导航
  changeNav(event) {
    // console.log(event.target.id) // 注意这里拿到的数字是字符串
    let navId = event.currentTarget.id //通过id向event传参的时候如果传的是number会自动转换成string
    // let navId = event.currentTarget.dataset.id // 使用data-传递的是数值型的 没有转换
    this.setData({
      navId: navId >>> 0 // 位移运算符 强制转换为数字
    })
  ...
  handleEnded(event) {
    ...
    videoUpdateTime.splice(videoUpdateTime.findIndex(item =>
      item.vid === event.currentTarget.id), 1)
    // 更新videoUpdateTime
    this.setData({
      videoUpdateTime
    })
